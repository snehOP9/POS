...
export const WaiterPage = () => {
  const {
    tables, orders, cart, cartTotal, addToCart, updateLineQuantity, selectedTableId, selectTable, adjustGuests,
    placeOrder, requestBill, serveOrder, setCartMode, menu, isMutating, refreshOperations, notify,
  } = usePos();
  ...
  return <main className="waiter-page">
    ...
    <nav className="waiter-mobile-nav" aria-label="Waiter mobile navigation"><button type="button" className="is-active" onClick={() => notify("Tables are visible in the floor overview.", "info")}><Table2 size={20} /><span>Tables</span></button><button type="button" onClick={() => notify(`${orders.length} active orders are reflected in the table cards.`, "info")}><ClipboardList size={20} /><span>Orders</span></button><button type="button" onClick={focusReadyTable}><ConciergeBell size={20} /><span>Ready</span>{readyOrders.length > 0 && <b>{readyOrders.length}</b>}</button><button type="button" onClick={() => notify(readyOrders.length ? `${readyOrders.length} ready table alerts.` : "No new floor alerts.", "info")}><BellRing size={20} /><span>Alerts</span></button><button type="button" onClick={() => notify("Maya Chen floor-service profile.", "info")}><ReceiptText size={20} /><span>Profile</span></button></nav>
  </main>;
};
