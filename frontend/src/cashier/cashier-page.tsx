...
import { BarChart3, Bell, Check, ChevronDown, CircleHelp, Clock3, CreditCard, LayoutGrid, LockKeyhole, MenuSquare, MoreHorizontal, Plus, ReceiptText, Search, Settings2, ShoppingBasket, Table2, UtensilsCrossed, WalletCards } from "lucide-react";
...
export const CashierPage = () => {
  ...
  const checkout = () => {
    placeOrder("cashier", paymentMethod === "Cash" ? "PAID" : "UNPAID");
    setCashTendered("");
  };
  ...
  return <main className="cashier-page">
    ...
    <section className="cashier-workspace">
      ...
      <div className="cashier-body">
        <section className="cashier-products"><div className="cashier-products__top"><label className="search-field search-field--operational"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search menu, SKU or item" /><kbd>F2</kbd></label><button type="button" className="outline-button"><ShoppingBasket size={17} /> Held orders <b>3</b></button></div><div className="cashier-categories">{cashierCategories.map((category) => <button type="button" key={category} className={category === activeCategory ? "cashier-category cashier-category--active" : "cashier-category"} onClick={() => setActiveCategory(category)}>{category}</button>)}</div><div className="cashier-grid">{filteredItems.map((item) => <button type="button" className="cashier-product" key={item.id} onClick={() => addToCart(item)} disabled={item.unavailable}><FoodVisual item={item} size="mini" /><div><span className={`dietary-dot dietary-dot--${item.dietary}`} /><strong>{item.name}</strong><small>{item.unavailable ? "Unavailable" : `${item.prepMinutes} min`}</small></div><b>{formatMoney(item.price)}</b><span className="cashier-product__add"><Plus size={16} /></span></button>)}</div>{!filteredItems.length && <div className="empty-state"><UtensilsCrossed size={30} /><strong>No live menu items are available.</strong><span>Menu data appears here after the API responds.</span></div>}</section>
        <aside className="pos-bill"><div className="pos-bill__top"><div><span className="eyebrow">Live bill</span><h2>{cart.length ? `${cart.length} menu selections` : "Start a fresh bill"}</h2></div><button type="button" className="quiet-button quiet-button--danger" onClick={clearCart} disabled={!cart.length || isMutating}>Clear</button></div><div className="order-mode-row">{(["DINE_IN", "PICKUP", "COUNTER"] as DiningMode[]).map((mode) => <button key={mode} type="button" className={cartMode === mode ? "order-mode order-mode--active" : "order-mode"} onClick={() => setCartMode(mode)}>{mode === "DINE_IN" ? "Dine in" : mode === "PICKUP" ? "Pickup" : "Counter"}</button>)}</div>{cartMode === "DINE_IN" && tables.length > 0 && <label className="table-selector"><Table2 size={17} /><span>Serving</span><select value={selectedTableId} onChange={(event) => selectTable(event.target.value)}>{tables.map((table) => <option key={table.id} value={table.id}>{table.label} · {table.guests || "new"} guests</option>)}</select><ChevronDown size={16} /></label>}
          ...
          <div className="payment-section"><div className="payment-section__heading"><span>Take payment</span><StatusPill status={cart.length ? "UNPAID" : "DRAFT"} subtle /></div><div className="payment-methods">{(["Cash", "UPI", "Card"] as const).map((method) => <button type="button" key={method} className={paymentMethod === method ? "payment-method payment-method--selected" : "payment-method"} onClick={() => setPaymentMethod(method)}>{method === "Cash" ? "₹" : method === "UPI" ? "UPI" : <CreditCard size={17} />}<span>{method}</span>{paymentMethod === method && <Check size={15} />}</button>)}</div>{paymentMethod === "Cash" && <div className="cash-input"><label htmlFor="cash-tendered">Cash received</label><div><span>₹</span><input id="cash-tendered" inputMode="numeric" value={cashTendered} onChange={(event) => setCashTendered(event.target.value.replace(/[^0-9]/g, ""))} placeholder="0" /></div>{tender > 0 && <small>Change to return: <b>{formatMoney(change)}</b></small>}</div>}</div>
          <button type="button" className="button button--saffron button--full pos-pay-button" onClick={checkout} disabled={isMutating || !cart.length || (paymentMethod === "Cash" && tender > 0 && tender < cartTotal)}><span>{isMutating ? "Saving securely…" : paymentMethod === "Cash" ? "Settle cash & send" : "Send · payment pending"}</span><strong>{formatMoney(cartTotal)}</strong></button><p className="pos-bill__footnote">{cartMode === "DINE_IN" && selectedTable ? `${selectedTable.label} · ${selectedTable.zone}` : "Walk-in order"} · Server calculates final price</p>
        </aside>
      </div>
    </section>
  </main>;
