...
export const CashierPage = () => {
  ...
  const dineInGuestCount = dineInTableSelected ? selectedTable?.guests ?? 0 : 0;
  ...
  const openCashierSection = (label: string) => {
    ...
    if (label === "Orders") {
      notify(activeOrders ? `${activeOrders} open orders are reflected in the register.` : "No open orders right now.", "info");
      return;
    }
    if (label === "Payments") {
  ...
  return <main className="cashier-page">
    ...
    <section className="cashier-workspace">
      ...
      <div className="cashier-body">
        ...
        <aside className="pos-bill"><div className="pos-bill__top"><div><span className="eyebrow">Live bill</span><h2>{cart.length ? `${cart.length} menu selections` : "Start a fresh bill"}</h2></div><button type="button" className="quiet-button quiet-button--danger" onClick={clearCart} disabled={!cart.length || isMutating}>Clear</button></div><div className="order-mode-row">{(["DINE_IN", "PICKUP", "COUNTER"] as DiningMode[]).map((mode) => <button key={mode} type="button" className={cartMode === mode ? "order-mode order-mode--active" : "order-mode"} onClick={() => { setCartMode(mode); if (mode !== "DINE_IN") setDineInTableSelected(false); }}>{mode === "DINE_IN" ? "Dine in" : mode === "PICKUP" ? "Pickup" : "Counter"}</button>)}</div>{cartMode === "DINE_IN" && tables.length > 0 && <><label className="table-selector"><Table2 size={17} /><span>Table</span><select value={dineInTableSelected ? selectedTableId : ""} onChange={(event) => { selectTable(event.target.value); setDineInTableSelected(true); }}><option value="" disabled>Select a table</option>{tables.map((table) => <option key={table.id} value={table.id}>{table.label}</option>)}</select><ChevronDown size={16} /></label>{dineInTableSelected && selectedTable && <div className="cashier-guest-control"><span>Guests for <b>{selectedTable.label}</b></span><div><button type="button" aria-label="Remove guest" onClick={() => adjustGuests(selectedTable.id, -1)} disabled={dineInGuestCount <= 0 || isMutating}><Minus size={15} /></button><b>{dineInGuestCount}</b><button type="button" aria-label="Add guest" onClick={() => adjustGuests(selectedTable.id, 1)} disabled={dineInGuestCount >= selectedTable.seats || isMutating}><Plus size={15} /></button></div></div>}</>}
